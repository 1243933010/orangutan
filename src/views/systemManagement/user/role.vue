      ...
      <el-dialog title="添加用户" center :visible.sync="dialogVisible" width="50%" :show-close="false">
          ...
          <el-form class="form" ref="formInfo" :inline="true" center  :rules="rules" size="small" :model="formInfo" label-width="100px">              
                  <el-form-item label="角色" prop="password">
                    <el-select multiple  v-model="formInfo.roles" placeholder="请选择角色">
                            <el-option :label="item.name" :value="item.code" v-for="(item, index) in roleList" :key="index" />
                        </el-select>
                  </el-form-item>
          </el-form>
          <div class="btn">
...
<script>
import { setUserRoles,roleList } from '@/api/project'
export default {
  data() {
      return {
          formInfo: {
  ...
  methods: {
    async getRole(){
            let res = await roleList({per_page:10000});
            console.log(res)
            if (res.code == 200) {
      ...
      async addMessage() {
          this.$refs.formInfo.validate(async(valid) => {
           if (valid) {
              let res  = await setUserRoles(this.formInfo); 
          console.log(res)
          if (res.code == 200) {
